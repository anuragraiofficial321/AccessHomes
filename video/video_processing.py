...
def process_video_first_per_class(video_path, detector, meta, mapped_plot, positions3d,
                                  ...
                                  global_use_transpose=None, global_z_sign=None, SAVE_DETECTED_CROPS=True, OUT_DEBUG_CSV=None, M_TO_FT=3.280839895013123):
...
#---------------------------Detect all the object in the input video-----------------------------#
def process_video_all_detections(video_path, detector, meta, mapped_plot, positions3d,
                                 s_map, R_map, t_map, chosen_proj,
                                 target_classes, save_detected=True, debug_reproject=False, reproject_csv=None,
                                 global_use_transpose=None, global_z_sign=None, SAVE_DETECTED_CROPS=True, OUT_DEBUG_CSV=None, M_TO_FT=3.280839895013123):
    """
    Process the video and return ALL detections (not only first-seen).
    Returns:
      - all_detections: list of detection dicts (one per detection)
      - per_class: dict mapping class_lower -> list of detection dicts

    Each detection dict contains keys similar to your first_seen 'info' dict:
      'class_name','video_frame_index','arkit_index','mapped_x','mapped_y',
      'object_x','object_y','object_yaw_deg','bbox','conf','crop_path','annotated_frame',
      'depth_model_units','distance_m','distance_ft'
    """
    # small logger helper (uses existing logger if available)
    ...
    def _log_debug(msg, *args):
        try:
            ...
            if args:
                ...
                pass  # keep debug quiet if no logger
    ...
    _log_debug("Video opened: %s, frames: %d size: %dx%d", video_path, total, vid_w, vid_h)
    map_fn = build_arkit_frame_index_map(meta, num_video_frames=total)

    target_set = None if target_classes is None else {c.lower() for c in target_classes}
    all_detections = []           # flat list of detections (every detection)
    per_class = {}                # class_lower -> [detections...]
    vf_idx = 0
    saved_frame_idxs = set()
    depth_cache = {}
    repro_records = []
    ...
    while True:
        ...
        try:
            ...
            _log_warning("Warning: detector failed on frame %d -> %s", vf_idx, e)
            dets = []
        if not dets:
            vf_idx += 1
            continue

        # For annotated frame (if saving), we write one annotated image per video frame (like before)
        annotated_frame = None
        if save_detected and vf_idx not in saved_frame_idxs:
            try:
                frame_copy = frame.copy()
                for dd in dets:
                    ...
                    cv2.putText(frame_copy, f"{dd['class_name']} {dd['conf']:.2f}",
                                (xx1, max(0, yy1-8)), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 2)
                annotated_frame = Path(str(video_path)).parent / f"annotated_vf{vf_idx}.jpg"
                cv2.imwrite(str(annotated_frame), frame_copy); annotated_frame = str(annotated_frame)
                saved_frame_idxs.add(vf_idx)
            except Exception as e:
                _log_warning("Warning: failed to save annotated frame for vf %d: %s", vf_idx, e)
                annotated_frame = None
            ...
            try:
                    ...
                    if cam_rot is not None:
                        ...
                        if cam_rot.shape != (3, 3):
                            if cam_rot.T.shape == (3, 3):
                                ...
                                _log_warning("Warning: cam_rot found but has unexpected shape %s -> ignoring", str(cam_rot.shape))
                                cam_rot = None
                except Exception as e:
                    _log_warning("Warning: error extracting cam_rot: %s", e)
                    cam_rot = None
                ...
                if save_detected:
                    try:
                        ...
                        if x2 > x1 and y2 > y1:
                            crop = frame[y1:y2, x1:x2]
                            crop_path = Path(str(video_path)).parent / f"{cls_l}_vf{vf_idx}_x{x1}_y{y1}.jpg"
                            cv2.imwrite(str(crop_path), crop); crop_path = str(crop_path)
                    except Exception as e:
                        _log_warning("Warning: failed to save crop for vf %d cls %s: %s", vf_idx, cls, e)
                        crop_path = None

                # depth via Zoe (cache per vf_idx)
                depth_val, distance_m, distance_ft = None, None, None
                try:
                    if vf_idx in depth_cache:
                        depth_map = depth_cache[vf_idx]
                    ...
                    x1, y1, x2, y2 = [int(round(v)) for v in d['xyxy']]
                    cx_bbox = int(round((x1 + x2) / 2)); cy_bbox = int(round((y1 + y2) / 2))
                    h_img_d, w_img_d = depth_map.shape[:2]
                    cx_bbox = _clamp(cx_bbox, 0, w_img_d-1); cy_bbox = _clamp(cy_bbox, 0, h_img_d-1)
                    patch = depth_map[max(0, cy_bbox-1):min(h_img_d-1, cy_bbox+1)+1,
                                      max(0, cx_bbox-1):min(w_img_d-1, cx_bbox+1)+1]
                    depth_val = float(np.median(patch.astype(float))) if patch.size > 0 else float(depth_map[cy_bbox, cx_bbox])
                    distance_m = depth_val; distance_ft = distance_m * M_TO_FT
                except Exception as e:
                    _log_warning("Warning: ZoeDepth failed for vf %d -> %s", vf_idx, e)
                    depth_val = None; distance_m = None; distance_ft = None

                # compute object world & mapped (same local compute function approach)
                obj_x, obj_y, obj_yaw = None, None, None
                p_world = None
                try:
                    from utils.intrinsics_utils import get_intrinsics_from_meta
                    ...
                    if chosen_R is not None and distance_m is not None:
                        ...
                        def compute_object_world_and_mapped_local(cam_pos3d, cam_rot, intrinsics_tuple, bbox_xyxy, depth_val,
                            ...
                            try:
                                ...
                                _log_warning("compute_object_world_and_mapped error: %s", e)
                                return None, None, None, None
                    ...
                    _log_warning("Warning: failed to compute object true position for vf %d cls %s: %s", vf_idx, cls, e)

                # Save info for this detection
                info = {
                    'class_name': cls,
                    'video_frame_index': vf_idx,
                    'arkit_index': a_idx,
                ...
                per_class.setdefault(cls_l, []).append(info)

                dist_print = f"{distance_m:.3f}m" if distance_m is not None else "N/A"
                _log_info("DETECTED: %s vf%d cam=(%.1f,%.1f) obj=(%s,%s) dist=%s yaw=%s z_sign=%s orient_conf=%s",
                          cls, vf_idx, mx, my, (obj_x if obj_x is not None else "N/A"), (obj_y if obj_y is not None else "N/A"),
                          dist_print, (obj_yaw if obj_yaw is not None else "N/A"), z_sign, orientation_conf)
            except Exception as e:
                _log_warning("Unhandled error processing detection in vf %d: %s", vf_idx, e)

        vf_idx += 1

    cap.release()

    # Save repro CSV if requested
    if debug_reproject and reproject_csv and repro_records:
        try:
            pd.DataFrame(repro_records).to_csv(reproject_csv, index=False)
            _log_info("Saved reprojection debug CSV: %s", reproject_csv)
        except Exception as e:
            _log_warning("Warning: failed to save reprojection CSV: %s", e)

    if debug_rows and OUT_DEBUG_CSV is not None:
        try:
            pd.DataFrame(debug_rows).to_csv(OUT_DEBUG_CSV, index=False)
            _log_info("Saved per-detection debug CSV: %s", OUT_DEBUG_CSV)
        except Exception as e:
            _log_warning("Warning: failed to save per-detection debug CSV: %s", e)

    return all_detections, per_class

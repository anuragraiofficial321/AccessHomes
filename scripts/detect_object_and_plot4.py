...
Adds plotting of fixed furniture from floor_plan.json (closets, sinks, toilets, cabinets, etc.)
alongside the existing floor polygons and first-seen detections.
"""
...
TARGET_CLASSES = ["couch","person","table","sink","chair","bed","tv"]
...
# ----------------- Helper functions -----------------
def reproject_error(p_obj_world, cam_pos3d, cam_rot, intrinsics_tuple, u, v, z_sign=1.0, frame_size=None):
    """
    Given a world point, compute reprojection error (euclidean pixel distance)
    between the provided image center (u,v) and the estimate from p_obj_world.
    Returns a large number if reprojection fails.
    """
    try:
        R = np.array(cam_rot, dtype=float)
        # point in camera coordinates
        p_cam = R.T @ (np.array(p_obj_world, dtype=float) - np.array(cam_pos3d, dtype=float))
        # get intrinsics / fallback
...
# ----------------- Main pipeline (updated plotting to draw fixed furniture) -----------------
def main():
    ...
    # --- robust loading / unpacking of floor polygons (fixes your ValueError) ---
    floor_load_result = load_floor_polygons()
    # handle common return shapes: tuple/list of at least 3, or dict with keys
    if isinstance(floor_load_result, (list, tuple)):
        ...
        if 'spaces' in floor_load_result and 'floor_min' in floor_load_result and 'floor_max' in floor_load_result:
            ...
            # attempt to compute bounds from spaces
    ...
    # ---- ADDED: load fixed furniture polygons to draw on the overlay ----
    try:
        fixed_furniture = load_fixed_furniture()
    except Exception as e:
